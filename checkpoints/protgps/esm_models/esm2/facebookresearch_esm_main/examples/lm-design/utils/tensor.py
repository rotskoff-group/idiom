...
def assert_shape(tensor, *shape_args):
    ...
    assert len(tensor.shape) == len(shape_args), (
        f"{source} should have {len(shape_args)} dimensions, actually has shape {tensor.shape}"
    )
    unbound = []
...
def assert_logprobs(tensor: torch.FloatTensor):
    ...
    try:
        ...
        raise AssertionError(
            "Not logprobs, perhaps you have logits and need to apply F.log_softmax?"
        )
...
def is_prob_tensor(p, atol=1e-3):
    return (
        ...
        and p.max() <= 1
        and torch.isclose(
            p.sum(axis=-1), torch.ones_like(p.sum(axis=-1)), atol=atol
        ).all()
    ).item()
