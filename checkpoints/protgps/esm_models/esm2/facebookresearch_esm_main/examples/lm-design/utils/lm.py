...
def lm_marginal(mlm, x, mask):
    ...
    assert torch.equal(
        b_coords, torch.repeat_interleave(torch.arange(B).to(x.device), n).to(x.device)
    )
...
class WrapLmEsm(WrapLM):
    ...
    def _prepare_seq(self, seq1h):
        ...
        seq1h = add_eos_bos(
            seq1h, bos_idx=self.vocab.cls_idx, eos_idx=self.vocab.eos_idx
        )
        seq_start_idx = 1
        seq_end_idx = L + 1
        ...
        if embed_dim != K:
            seq1h = torch.cat(
                [seq1h, torch.zeros(B, L + 2, embed_dim - K).to(seq1h)], -1
            )
