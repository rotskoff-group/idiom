...
class Designer:
    ...
    def __init__(self, cfg, target_pdb_path=None, device=None):
        ...
        self.allowed_AA = "".join(
            ...
            if (("suppress_AA" not in self.cfg) or (AA not in self.cfg.suppress_AA))
        )
        ...
        torch.backends.cudnn.benchmark = (
            True  # Slightly faster runtime for optimization
        )
    ...
    def _init_target(self, pdb_path):
        ...
        self.pos_idx = (
            torch.tensor(self.target_data["idx"]).long()[None,].to(self.device)
        )
        self.coords = (
            torch.tensor(self.target_data["coords6d"]).long()[None,].to(self.device)
        )
        ...
        self.target_contacts = (self.target_distancemap < self.cutoff_dist).to(
            self.device
        )
    ...
    def init_sequences(self, num_seqs):
        ...
        bt = torch.from_numpy(
            np.random.choice(AA_indices.cpu().numpy(), size=(B, self.L))
        ).to(self.device)
    ...
    def calc_structure_loss(self, x_seq, temp_struct=None):
        ...
        for i, targetname in enumerate(targets):
            ...
            cce_pos = get_cce_loss(
                res_preds[f"p_{targetname}"], target, target_pos_mask
            )
            cce_neg = get_cce_loss(
                res_preds[f"p_{targetname}"], target, target_neg_mask
            )
    ...
    def calc_total_loss(
        self, x, mask, LM_w, struct_w, ngram_w, ngram_orders, temp_struct=None
    ):
        """
    ...
    def run_from_cfg(self):
        ...
        logger.info("Final designed sequences:")
        ...
        if not self.resuming_stage:
            for name, maybe_sspec in walk_cfg(cfg, sep="."):
                if is_sspec(maybe_sspec):
                    assert name not in self.schedulers, f"Trying to re-register {name}"
    ...
    def init_schedulers(self, **kwargs):
        ...
        assert sys.version_info >= (3, 6), (
            "py>=3.6 preserve kwarg and dict order see PEP468"
        )
