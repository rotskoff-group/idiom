...
#
# PADDLE-noSS runs from sequence alone; its TensorFlow environment is built by uv from the script
# header, and the model files (36 MB) are cloned once into IDIOM_PADDLE_DIR.
#
# The raw reward is a Z-score against PADDLE's background, so it is already on a usable scale: base
# generations sit at +0.9 (sd 1.7) and strong natural activation domains at 3-8. Left unshaped to
# maximize activation strength; the weight is what keeps a strong domain from dwarfing the
# guardrails. About 3 s per step, so give it a long timeout.
###

# Repo root: where sbatch was submitted from, or this script's own location under bash.
REPO="${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)}"
OUT="${IDIOM_OUT:-$HOME/idiom-runs}/grpo-paddle"   # EDIT: keep runs on scratch, not in the repo

WEIGHT=0.5                                        # EDIT: Z-scale is unbounded above; keep this modest
TIMEOUT=600.0
SCORER="uv run --script $REPO/cookbook/rewards/scorers/paddle.py"
