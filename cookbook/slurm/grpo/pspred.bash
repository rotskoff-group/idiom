#!/bin/bash
#SBATCH --job-name=idiom-grpo-pspred
...
###
# GRPO toward phase-separation thermodynamics, scored by PSpred (trained on CALVADOS simulations).
#
# Unlike a classifier score the raw reward is a physical quantity, so a target means something:
# --target dG is transfer free energy in kT, where more negative phase-separates more readily. Base
# generations average -0.1 (sd 0.8) and LAF1, a 170-residue LLPS driver, reaches -6.1 -- so -3 is
# demanding but reachable at the default length. --target logcdil_mgml gives saturation concentration
# instead. The predictor's files (3.7 MB) are fetched once into IDIOM_PSPRED_DIR.
###

# Repo root: where sbatch was submitted from, or this script's own location under bash.
REPO="${SLURM_SUBMIT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)}"
OUT="${IDIOM_OUT:-$HOME/idiom-runs}/grpo-pspred"   # EDIT: keep runs on scratch, not in the repo

TARGET_KIND=dG                                    # EDIT: dG | logcdil_mgml | cdil_mgml
TARGET=-3.0                                       # EDIT: in kT for dG
WIDTH=1.0                                         # EDIT: absolute here, since dG crosses zero
WEIGHT=1.0
SCORER="uv run --script $REPO/cookbook/rewards/scorers/pspred.py --target $TARGET_KIND"
