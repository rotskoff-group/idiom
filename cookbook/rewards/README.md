...
To build a signature, use [feature_enrichment.ipynb](../notebooks/feature_enrichment.ipynb) or
[feature_enrichment.bash](../scripts/feature_enrichment.bash). In the GRPO script, set `FEATURES`
to the exported JSON path, `SIGNATURE` to its signature name, and `CASE` to its case (usually `top30`).
...
Return one finite score per sequence, in input order, including empty sequences. IDiom rejects
incorrect output counts and non-finite raw scores, shaped scores, weights, or totals.
For a function that scores one sequence, `batchify` supplies the batch wrapper:
...
Python library output, and prevents the script filename from shadowing the package it imports.
Scorers do not need to import IDiom.
