...
writes the strongest as a signature, and shows the residue grammar behind each one.
    ...
    uv run examples/scripts/feature_enrichment.py
...
POSITIVE = str(REPO / "examples/example_data/protgps/nucleolus.fasta")  # the set you care about
...
# sbatch examples/slurm/grpo.bash.
