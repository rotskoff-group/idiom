"""Extractor tests: residue-only selection, marker drop, alignment, multi-layer."""
...
def test_residue_only_selection_and_alignment():
    """Verify that extracted rows align with residue tokens and their positions."""
...
def test_keep_markers_option():
    """Verify that extraction can retain FIM marker positions."""
    model = IDiomTransformer(TINY).eval()
