...
def test_steering_context_only_edits_residues():
    """Verify that a region-masked steering hook changes model logits."""
...
def test_steer_generation_runs():
    """Verify batch size and token limits for steered generation."""
    model = IDiomTransformer(TINY).eval()
...
def test_ablation_respects_scalar_strength(strength):
    """Verify scaled feature subtraction, including identity behavior at zero strength."""
...
def test_zero_ablation_generation_matches_unsteered_baseline():
    """Verify that zero-strength ablation matches unsteered generation."""
