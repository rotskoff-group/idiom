...
sae_signature is a factory: a term names it as a "module:function" reward and passes its settings
in params, so which signature a run chases, and which SAE and signature file it is read from, are
recorded in the run's config alongside every other term.
...
# and from any working directory; point a term's params.features at your own JSON of the same shape
...
def _target_ids(signature: str, features: str, case: str, sae_dir: str, device: str | None):
    ...
    if signature not in sets:
        raise KeyError(f"signature {signature!r} not in case {case!r} of {features} "
                       f"(available: {sorted(sets)}); set the term's params.signature")
...
def sae_signature(signature: str, *, features: str = DEFAULT_FEATURES, case: str = DEFAULT_CASE,
                  sae: str = DEFAULT_SAE, device: str | None = None) -> Callable[[str], float]:
    """Build the feature-match reward for one signature, from a term's params.
    ...
    Returns:
        Callable[[str], float]: A per-idr reward in [0, 1].
    ...
    try:
        ...
        raise ValueError(
            ...
            f"term's params.features at it.") from e
    if signature not in sets:
    ...
    def reward(idr: str) -> float:
        return feature_match(idr, signature, features=features, case=case, sae=sae, device=device)
