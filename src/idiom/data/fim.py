...
per sample (the idr/idp choice is a runtime augmentation, replacing the old stored ×2):

- :func:`fim_idr`  ``1{prefix}3{suffix}2{IDR}``  — context-prompted (IDR in its protein)
- :func:`fim_idp`  ``132{IDR}``                  — de-novo (no flanking context, standalone IDP)
...
def fim_idr(seq: str, start: int, end: int) -> str:
...
def fim_idp(seq: str, start: int, end: int) -> str:
    """``132{IDR}`` — empty prefix and suffix, i.e. de-novo (standalone IDP) generation."""
...
def fim_prompt(seq: str = "", start: int = 0, end: int = 0) -> str:
    ...
    With the default empty sequence this is ``"132"`` (de-novo / standalone IDP). Given a protein + IDR span
...
def residue_source_positions(seq_len: int, start: int, end: int, variant: str = "idr") -> list[int]:
    ...
    ``idr`` (context) that is ``prefix, suffix, IDR``; for ``idp`` (de-novo) it is just the IDR.
    Pairing these indices with the residue-only activation rows aligns every vector to its residue
    (and the IDR residues are exactly those with ``start <= pos < end``).
    """
    if variant == "idr":
        return [*range(0, start), *range(end, seq_len), *range(start, end)]  # prefix, suffix, IDR
    if variant == "idp":
        return list(range(start, end))  # IDR only
    raise ValueError(f"variant must be 'idr' or 'idp', got {variant!r}")
